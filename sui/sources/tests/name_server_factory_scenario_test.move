#[test_only]
module smartinscription::name_server_factory_scenario_test{
    use std::option;
    use sui::clock;
    use sui::sui::SUI;
    use sui::coin;
    ...
    use smartinscription::name_server_factory;
    ...
    public fun test_whole_process() {
        ...
        test_scenario::next_tx(scenario, admin);
        let tick_name_tick_record = {
            name_server_factory::deploy(&mut deploy_record, test_scenario::ctx(scenario));
        ...
        test_scenario::next_tx(scenario, usera);
        {
            ...
            let test_sui = coin::mint_for_testing<SUI>(name_server_factory::init_locked_sui(), test_scenario::ctx(scenario));
            name_server_factory::mint(&mut tick_name_tick_record, test_sui, test_tick_name, &c, test_scenario::ctx(scenario)); 
        };
        ...
        test_scenario::next_tx(scenario, usera);
        {
            ...
            assert!(std::ascii::into_bytes(movescription::tick(&first_inscription)) == name_server_factory::tick(), 2);
            ...
            let tick_record = movescription::do_deploy_with_witness(&mut deploy_record, first_inscription, test_tick_total_supply, WITNESS{}, test_scenario::ctx(scenario));
